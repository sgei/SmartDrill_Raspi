...
fs.watchFile(config.path + "drill.dat", function(eventName, filename){
  ...
  fs.readFile(config.path + "drill.dat", "utf8", function(err, data){
    ...
    if(err){
      ...
      // old parsing of drill.dat
      /*
      // convert object to string and split string by "\n" into array
      // use array to delete unwanted data
      var usage_1 = parseInt(usage_raw[5].replace("USAGE_TIME_MODE_1=", ""));
      var usage_2 = parseInt(usage_raw[6].replace("USAGE_TIME_MODE_2=", ""));
      var usage_3 = parseInt(usage_raw[7].replace("USAGE_TIME_MODE_3=", ""));
      */

      // new parsing of drill.dat
      ...
      var time_usage =
      (
        ((usage_1 * config.payment.usage_multiplicator_0) +
        (usage_2 * config.payment.usage_multiplicator_1) +
        (usage_3 * config.payment.usage_multiplicator_2))
        * (10**18)
      );
            ...
            function(err, transactionHash){
              if(!err){
                ...
                fs.unlink(config.path + "lock_local", function(err){
                  if(err){"eth-pi: something went wrong while deleting"}
                  else{console.log("eth-pi: lock file has been deleted");}
                })
              }
              else{
